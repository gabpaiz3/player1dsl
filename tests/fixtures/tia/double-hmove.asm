...
; just positioned?
;
; QUESTION: `PosObjectX` ends `sta WSYNC / sta HMOVE / rts` and is called once
; per object with no HMCLR between the calls. On real hardware HMOVE applies
; every HMxx register that is currently set, so the SECOND call's HMOVE
; re-applies the FIRST object's fine adjustment and P0 ends up displaced twice.
; Does it?
;
...
;
; Its control is double-hmove-cleared.asm, identical but for an HMCLR between
; the two calls. That one must be BLACK in both implementations, which is what
; says this fixture is capable of reporting black at all.
; ---------------------------------------------------------------------------
